...
class Solution:
    def minDifficulty(self, jobDifficulty: List[int], d: int) -> int:
        return self.bottomUp(jobDifficulty, d)
    
    def bottomUp(self, jobDifficulty: List[int], d: int) -> int:
        n = len(jobDifficulty)
    ...
    def topDown(self, jobDifficulty: List[int], d: int) -> int:
        n = len(jobDifficulty)
        ...
        for i in range(n-1,-1,-1):
            ...
            hardestJobRem[i] = hardest
        
        @lru_cache(None)
        def dp(i, day):
            if day == d:
                return hardestJobRem[i]
            
            minDifficulty = float("inf")
            hardest = 0
            for j in range(i, n - (d - day)):
                hardest = max(hardest, jobDifficulty[j])
                minDifficulty = min(minDifficulty, hardest + dp(j+1, day+1))
