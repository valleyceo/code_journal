...
# Where d is max number, n is array length, b is base of numbering system
def radixSort(array):
    if len(array) == 0:
		return array
...
def countingSort(array, digit):
	...
	# place a counter
	for num in array:
		countArray[(num  // digitColumn) % 10] += 1
	
	# accumulate sum - count value equals idx to be placed
